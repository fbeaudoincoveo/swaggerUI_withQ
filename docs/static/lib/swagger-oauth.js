...
function handleLogin() {
  ...
  if(auths) {
    ...
    for(key in defs) {
      var auth = defs[key];
      if(key != 'v7' && auth.type === 'oauth2' && auth.scopes) {
        oauth2KeyName = key;
        var scope;
  ...
  for (i = 0; i < scopes.length; i ++) {
    scope = scopes[i];
    str = '<li><input type="checkbox" id="scope_' + i + '" scope="' + scope.scope + '" checked/>' + '<label for="scope_' + i + '">' + scope.scope;
  ...
  popupDialog.find('button.api-popup-authbtn').click(function() {
    ...
    var pathname = location.pathname.endsWith("index.html") ? location.pathname.substring(0, location.pathname.lastIndexOf("/")) : location.pathname;
